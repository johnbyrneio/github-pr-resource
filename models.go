...
func (s *Source) Validate() error {
	if s.AccessToken == "" && (s.AppID == 0 || s.InstallationID == 0 || s.PrivateKey == "") {
		return errors.New("access_token or app_id, installation_id, and private_key must be set")
	}
	if s.AccessToken != "" && (s.AppID != 0 || s.InstallationID != 0 || s.PrivateKey != "") {
		return errors.New("access_token cannot be set with app_id, installation_id, and private_key")
	}
	if s.Repository == "" {
